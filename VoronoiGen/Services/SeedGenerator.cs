...
namespace VoronoiGen.Services
{
    ...
    public static class SeedGenerator
    {
        ...
        public static List<Vector2> GenerateRandom(Polygon boundary, List<Polygon>? holes, bool ignoreHoles, int count, int? rngSeed = null)
        {
            ...
            while (seeds.Count < count && attempts < count * 500)
            {
                attempts++;
        ...
        public static List<Vector2> GeneratePoisson(Polygon boundary, List<Polygon>? holes, bool ignoreHoles, float radius, int? rngSeed = null, int k = 30)
        {
            var rand = rngSeed.HasValue ? new Random(rngSeed.Value) : new Random();
            var bounds = boundary.GetBounds();
